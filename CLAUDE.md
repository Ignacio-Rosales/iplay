...
- **Admin (hidden route)**: `AdminPage.jsx` is a self-contained page (no shared layout) gated by a password compared against `VITE_ADMIN_PASSWORD` (client-side only — the password ships in the bundle, this is not real per-user auth). On a correct password it also silently signs in to a single fixed Firebase Auth account via `loginAdmin()` (`src/services/firebase.js`, using `VITE_ADMIN_AUTH_EMAIL`/`VITE_ADMIN_AUTH_PASSWORD`), so Firestore security rules can require `request.auth != null` on writes instead of allowing open writes from anyone who finds the `firebaseConfig` in the bundle; `logoutAdmin()` signs out on "Cerrar sesión". Once "authenticated", it shows a store-settings form (title/tagline/WhatsApp number/logo upload → `updateStoreSettings`), a product form that both creates (`addProduct`) and edits (`updateProduct`, image re-upload optional when editing) products, and the product list with edit/delete actions.
...
- `VITE_ADMIN_AUTH_EMAIL`, `VITE_ADMIN_AUTH_PASSWORD` — credentials of the single fixed Firebase Auth user that `/admin` silently signs in as after the password gate, so Firestore rules can require `request.auth != null` on writes (see Authentication > Sign-in method > Email/Password and Authentication > Users in the Firebase console)
